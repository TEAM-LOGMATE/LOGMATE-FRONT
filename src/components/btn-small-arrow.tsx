...
export default function BtnSmallArrow({ direction = 'up' }: BtnSmallArrowProps) {
  const rotationClass =
    direction === 'up' ? 'rotate-[360deg]' : 'rotate-180';
  ...
  return (
    ...
    >
      <svg
        ...
        viewBox="0 0 14 9"
        fill="none"
        className={`w-[8.231px] h-[13.5px] flex-shrink-0 ${rotationClass}`}
      >
        <path
          ...
          className="fill-[#535353] group-hover:fill-[#F2F2F2] transition-colors"
        />
      </svg>
    </button>
  );
