...
interface MultilineSettingsProps {
  value: {
    enabled: boolean;
    maxLines: number;
  };
  onChange: (newValue: MultilineSettingsProps["value"]) => void;
}

export default function MultilineSettings({ value, onChange }: MultilineSettingsProps) {
  const toggleChecked = () => {
    onChange({ ...value, enabled: !value.enabled });
  };

  const handleIncrease = () => {
    onChange({ ...value, maxLines: value.maxLines + 1 });
  };

  const handleDecrease = () => {
    const next = value.maxLines - 1;
    onChange({ ...value, maxLines: next > 1 ? next : 1 });
  };
      ...
      {value.enabled && (
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1 h-[48px]">
            <Input48
              value={String(value.maxLines)}
              onChange={(e) =>
                onChange({ ...value, maxLines: Number(e.target.value) || 1 })
              }
              placeholder="10"
              align="center"
            />
